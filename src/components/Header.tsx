...
export default function Header() {
  ...
  const businessName = "Daves Motors"
  const category = "Garage"
          ...
          <div className="flex justify-between items-center py-2 text-sm">
            <div className="flex items-center space-x-6">
              ...
              <div className="flex items-center space-x-2">
                ...
                <span className={`${scrolled ? 'text-gray-600' : 'text-white/80'}`}>info@daves motors.com</span>
              </div>
            </div>
            <div className="flex items-center space-x-2">
        ...
        <div className="flex justify-between items-center h-16">
          ...
          >
            ...
            <div className="flex flex-col">
              ...
              <span className={`text-xs ${scrolled ? 'text-gray-500' : 'text-white/70'}`}>
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              className="md:hidden bg-white rounded-xl shadow-2xl mt-2 p-6 border border-gray-100"
            >
              <nav className="flex flex-col space-y-4">
                {navItems.map((item) => (
                  <motion.button
                    ...
                    onClick={() => scrollToSection(item.href.replace('#', ''))}
                    whileTap={{ scale: 0.95 }}
                    className="text-left text-gray-700 hover:text-primary font-medium py-2 border-b border-gray-100 last:border-b-0"
                  >
                    {item.name}
                  </motion.button>
                ))}
                <motion.button
                  whileTap={{ scale: 0.95 }}
                  onClick={() => scrollToSection('contact')}
                  className="bg-primary text-white px-6 py-3 rounded-xl font-medium hover:bg-primary/90 transition-colors mt-4"
                >
                  Book Now
                </motion.button>
              </nav>
